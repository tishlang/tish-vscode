...
- **Single language mode:** `tish-jsx` / separate TextMate bundle removed. One grammar (`source.tish`) includes JSX; **`.tish`** and **`.tishx`** both map to language id **`tish`**. LSP and format-on-save apply uniformly.
...
- Initial release: TextMate grammars for `.tish` / `.tishx`, snippets, problem matchers (`tish-compile`, `tish-rustc`).
